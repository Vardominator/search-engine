...
def test_wildcard_handles_star_after():
    answer = {'test'}
    assert answer == specialqueries.wildcard_query('tes*', k_index)

def test_handles_splitting_long_grams():
    answer = {'tempest'}
    assert answer == specialqueries.wildcard_query('tempe*', k_index)

def test_wildcard_star_after():
    answer = {'best', 'test', 'tempest'}
    assert answer == specialqueries.wildcard_query('*est', k_index)

def test_wildcard_small_grams():
    answer = {'test', 'best', 'tempest'}
    assert answer == specialqueries.wildcard_query('*t', k_index)

def test_wildcard_middle_star():
    answer = {'test', 'tempest'}
    assert answer == specialqueries.wildcard_query('te*t', k_index)

def test_wildcard_multiple_stars():
    answer = {'tempest'}
    assert answer == specialqueries.wildcard_query('te*pe*t', k_index)
